...
zeitdauerkategoriendf <- data.frame(Zeitdauer = zeitdauer) # Dataframe spaeter mit Kategorien
...
ggplot(zeitdauerdf, aes(x = zeitdauer)) +
  ...
  labs(title = "Verteilung der beobachteten Werte", x = "Beobachtete Werte real", y = "Dichte") +
  theme_minimal() +
...
hist_data <- hist(zeitdauerdf$zeitdauer, breaks = 100, plot = FALSE) # Bins = Fehler
...
```
Der Test zeigt dass die Häufigkeiten signifikant von den erwarteten Häufigkeiten abweichen. Da p nahezu null ist, bedeutet dies, dass es extrem unwahrscheinlich ist, dass die beobachteten Häufigkeiten von den erwarteten Häufigkeiten abweichen, nur aufgrund von Zufälligkeiten. Die Abweichung ist statistisch signifikant. Es gibt 18 Freiheitsgrade somit unabhaengige Werte in der Berechnung.

Aufgabe 2 a)
Datenn: Aktuelle Messwerte, 10 Parameter (Format csv)
...
# Scatterplot matrix mit vier werten
...
```
Diskussion:
In den unteren Plots erkennt man ob die ausgewaehlten Variablen einen Zusammenhang. In den oberen sieht man die Korrelation. Die Werte deuten auf keine Korrelation hin. Die letzte Grafik zeigt die Dichteverteilung der Datenpunkte sowie die Dichtekurven. Zwei Variablen haben keine regelmaessigen Werte. Niederschlag und Sonnenscheindauer. Zwei Werte zeigen eine Gausskurve. Temperatur und Taupunkt. Diese haengen zusammen. Die relative Feuchtigkeit und die Globalstrahlung.
...
# Lineare Regression
model <- lm(data_meteo$tre200s0 ~ data_meteo$tde200s0, data_meteo)
...
```
Residual-Plot: Die Punkte sind gestreut, dafuer regelmaessig und nicht bei der 0-Kureve. Es gibt jedoch ein Trend das es mehr Wrte zwischen 10 - 15 gibt. Dies deutet auf Heteroskedastizität hin.

Der QQ-Plot zeigt dass die Punkte auf der Diagonale liegen. Vorallem innerhalb der Standardabweichung von -1 und +1. Daneben zeigt die Grafik, dass die Punkte von der Normalverteilung abweichen.

Aufgabe 2 d)
Bereits unterhalb der einzelnen Aufgaben gmacht.

Zusaetzlicher Scatter-Plot:
...
# Scstterplot von zwei Variablen 'chargingPower_10kW_count' and 'chargingPower_21kW_count'
