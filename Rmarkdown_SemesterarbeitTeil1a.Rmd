...
audiofile_long <- readWave("1727.wav")

# Fehlende Werte entfernen
audiofile_long <- na.omit(audiofile_long)

# Audiodatei kuerzen
start_time <- 2000  # Start bei 2000
end_time <- 152000  # Ende nach 152000
trimmed_audio <- extractWave(audiofile_long, from = start_time, to = end_time)
writeWave(trimmed_audio, "1727_gekuerzt.wav")

# Path wav file, source kaggle - 1 min
audiofile_short <- readWave("1727_gekuerzt.wav")

# Structure audio file
str(audiofile_short)

# Einzelne Schalldruckwerte des monokanals konvertieren in Werte von -1 bis 1
left_channel <- audiofile_short@left
s1 <- left_channel / 2^(audiofile_short@bit -1) # 32 Bit - 1
plot(s1)

# Plotten der Druckwerte nach der Zeitachse
timeArray <- (0:(150000-1)) / audiofile_short@samp.rate
...
1. Forschungsfrage: Es handelt sich um ein Instrument
2. Forschungsfrage: Audioumwandlung beeinflusst die qualitaet nicht

```{r}
audio_file <- readWave("1727_gekuerzt.wav")

# Histogramme
hist(audio_file, main = "Histogramm der Daten", xlab = "Amlitude", ylab = "Häufigkeit", col = "skyblue", border = "black")

# Balkendiagramm
freq_table <- table(cut(audio_file, breaks = seq(min(data), max(data), by = 2)))

# Erstellung einer Häufigkeitstabelle
barplot(freq_table, main = "Balkendiagramm der Daten", xlab = "Intervalle", ylab = "Häufigkeit", col = "lightgreen")

# Boxplots
boxplot(audio_file, main = "Boxplot der Daten", ylab = "Werte", col = "lightblue")
...
# Funktion zum Erstellen eines Histogramms aus Audiodaten
create_audio_histogram <- function(audio_file) {
  ...
  # Histogramm der Audiodaten erstellen
  hist(audio_mono, main = "Histogramm der Audiodaten", xlab = "Amplitude", ylab = "Häufigkeit", col = "skyblue", border = "black")
}

# Pfad zur Audiodatei
audio_file <- "1727_gekuerzt.wav"

# Histogramm der Audiodaten erstellen
create_audio_histogram(audio_file)
